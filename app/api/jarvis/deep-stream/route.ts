...
async function loadCat(cat: string): Promise<Record<string, Function>> {
  ...
  try {
    const catMap: Record<string, () => Promise<Record<string, any>>> = {
      ...
      books:         () => import('../../../../lib/tools/categories/books'),
      dev:           () => import('../../../../lib/tools/categories/dev'),
      media:         () => import('../../../../lib/tools/categories/media'),
      music:         () => import('../../../../lib/tools/categories/music'),
    }
        ...
        if (polRes.ok && polRes.body) {
          ...
          while(true) {
            ...
            for(const line of lines) {
              ...
              try { const d=JSON.parse(line.slice(6)); const tok=d.choices?.[0]?.delta?.content||''; if(tok) send({type:'token',token:tok}) }
              catch { const t=line.slice(6).trim(); if(t&&t!=='[DONE]') send({type:'token',token:t+' '}) }
            }
          }
          send({ type: 'done', toolsUsed: [], reply: '', card: null })
        } else {
          send({ type: 'error', message: 'Free AI unavailable. Gemini key add karo: /connected' })
        }
      } catch {
        send({ type: 'error', message: 'Free AI timeout. Dobara try karo.' })
      }
      w.close(); return
    }
